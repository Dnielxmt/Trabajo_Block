...
async function verDetallePedido(idPedido) {
    try {
        ...
        let html = `
            ...
            <p>Estado: ${["Pendiente","Enviado","Recibido","Cancelado"][estado]}</p>
            <p>Descuento: ${descuento}%</p>
            <p>Fecha: ${new Date(Number(fecha) * 1000).toLocaleString()}</p>
            <h4>Productos</h4>
            <ul>
        `;

        for (let i = 0; i < idsProductos.length; i++) {
            html += `<li>Producto ID ${idsProductos[i]} – Cantidad ${cantidades[i]}</li>`;
        }
